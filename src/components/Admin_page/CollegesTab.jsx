        ...
        <div>
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
                <div className="flex items-center gap-3">
                    ...
                    <h2 className="text-lg font-bold text-white">Pending Verification</h2>
                </div>
                    ...
                    <table className="w-full admin-table">
                        <thead>
                            <tr className="bg-white/[0.02]">
                                <th className="text-left py-4 px-4 text-xs font-semibold uppercase tracking-wider text-slate-400 font-mono">Institution</th>
                                <th className="text-left py-4 px-4 text-xs font-semibold uppercase tracking-wider text-slate-400 font-mono">Location</th>
                                <th className="text-center py-4 px-4 text-xs font-semibold uppercase tracking-wider text-slate-400 font-mono">Type</th>
                                <th className="text-right py-4 px-4 text-xs font-semibold uppercase tracking-wider text-slate-400 font-mono">Actions</th>
                            </tr>
                        </thead>
                        <tbody>
                            {loading ? (
                                Array.from({ length: 3 }).map((_, i) => (
                                    <tr key={i}>
                                        <td colSpan={4}><div className="h-16 bg-slate-700/30 rounded animate-pulse m-2" /></td>
                                    </tr>
                                ))
                            ) : colleges.length === 0 ? (
                                <tr>
                                    <td colSpan={4} className="text-center text-slate-500 py-12 flex flex-col items-center justify-center">
                                ...
                                <tr key={college._id} className="group hover:bg-white/[0.02] transition-colors">
                                    <td className="px-4 py-4">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                <p className="font-medium text-white text-sm">{college.name}</p>
                                    ...
                                    <td className="px-4 py-4">
                                        <div className="flex items-center gap-2 text-sm text-slate-300">
                                            ...
                                            <span className="ml-1 text-xs text-slate-500">({college.country})</span>
                                        </div>
                                    </td>
